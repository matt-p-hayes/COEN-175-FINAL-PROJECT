...
Type Type::promote() const {
    if(_specifier == CHAR && _indirection == 0) {
        return Type(INT, 0);
...
bool Type::isCompatibleWith(const Type &that) const {
    Type thisPromoted = this->promote();
    Type thatPromoted = that.promote();

    if(thisPromoted.isNumeric() && thatPromoted.isNumeric()) {
        return true;
    }
    if(thisPromoted.isPointer() && thatPromoted.isPointer() && (thisPromoted.specifier() == thatPromoted.specifier()) && (thisPromoted.indirection() == thatPromoted.indirection())) {
        return true;
    }
    if(thisPromoted.isPointer() && thatPromoted.isPointer() && (thisPromoted.specifier() == VOID && thisPromoted.indirection() == 1)) {
        return true;
    }
    if(thisPromoted.isPointer() && thatPromoted.isPointer() && (thatPromoted.specifier() == VOID && thatPromoted.indirection() == 1)) {
        return true;
    }
